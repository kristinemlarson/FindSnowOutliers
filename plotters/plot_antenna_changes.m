function [h_out]=plot_antenna_changes(site_ind,snr_ind,fignum)
%PLOT_ANTENNA_CHANGES - Plots a vertical line for each antenna change
%
% Syntax:  [h_out]=plot_antenna_changes(site_ind,snr_ind,fignum)
%
% Inputs:
...
% Global Parameters:
%   format
%       .line_style_antenna_marks  default '--'
%       .line_width_antenna_marks  default 1
%       .color.antenna_marks      default 'k'
%
% Outputs:
%   h_out               Plot Handle
%
...
%% Get Offset Data
if(isfield(sites_list(site_ind),'antenna_changes'))
    changes=sites_list(site_ind).antenna_changes;
else
    changes=zeros(0,3);
end
if(isempty(changes))
...
% Color
if(isfield(format.color,'antenna_marks'))             % Specific
    color=format.color.antenna_marks(:,snr_ind);
