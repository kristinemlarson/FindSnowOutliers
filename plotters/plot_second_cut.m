function [h_out]=plot_second_cut(site_ind,snr_ind,fignum)
%PLOT_SECOND_CUT - Plots SNR data after the second cut
%
% Syntax:  [h_out]=plot_second_cut(site_ind,snr_ind,fignum)
%
% Inputs:
...
% Global Parameters:
%   format
%       .line_style_cut2        default 'none'
%       .marker_cut2            default '.'
%       .marker_size_cut2       default 6
%       .color.cut2             default 'k'
%
% Outputs:
%   h_out               Plot Handle
%
...
%% Get SNR Data
if(isfield(sites_list(site_ind).snr{snr_ind},'second_cut'))
    data=sites_list(site_ind).snr{snr_ind}.second_cut;
else
    data=zeros(0,4);
end
...
% Line Style
if(isfield(format,'line_style_cut2'))               % Specific
    line_style=format.line_style_cut2;
...
% Marker
if(isfield(format,'marker_cut2'))               % Specific
    marker=format.marker_cut2;
...
% Marker Size
if(isfield(format,'marker_size_cut2'))              % Specific
    marker_size=format.marker_size_cut2;
...
% Color
if(isfield(format.color,'cut2'))                    % Specific
    color=format.color.cut2(:,snr_ind);
