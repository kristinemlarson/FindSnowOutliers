function [h_out]=plot_bad_second_cut(site_ind,snr_ind,fignum)
%PLOT_BAD_SECOND_CUT - Plots the days marked as bad during the second cut
%
% Syntax:  [h_out]=plot_bad_second_cut(site_ind,snr_ind,fignum)
%
% Inputs:
...
% Global Parameters:
%   format
%       .line_style_bad_cut2    default 'none'
%       .marker_bad_cut2        default '.'
%       .marker_size_bad_cut2   default 6
%       .color.bad_cut2         default 'g'
%
% Outputs:
%   h_out               Plot Handle
%
...
%% Get SNR_Data
if(isfield(sites_list(site_ind).snr{snr_ind},'bad_second_cut'))
    data=sites_list(site_ind).snr{snr_ind}.bad_second_cut;
else
    data=zeros(0,4);
end
...
% Line Style
if(isfield(format,'line_style_bad_cut2'))           % Specific
    line_style=format.line_style_bad_cut2;
...
% Marker
if(isfield(format,'marker_bad_cut2'))           % Specific
    marker=format.marker_bad_cut2;
...
% Marker Size
if(isfield(format,'marker_size_bad_cut2'))          % Specific
    marker_size=format.marker_size_bad_cut2;
...
% Color
if(isfield(format.color,'bad_cut2'))                % Specific
    color=format.color.bad_cut2(:,snr_ind);
